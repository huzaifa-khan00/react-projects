...
function Product() {
  ...
  let fetchProductDetails = async () => {
    try {
      let response = await axios.get(`https://dummyjson.com/products/${prodId}`);
        ...
        <div className="flex flex-wrap -mx-4">
          ...
          <div className="w-full md:w-1/2 px-4">
            ...
            <div className="flex items-center mb-4">
              {[...Array(Math.round(productData?.rating || 0))].map((_, i) => (
                <svg
                  key={i}
              ...
              <span className="ml-2 text-gray-600">
                {productData?.rating} ({productData?.reviews?.length || 0} reviews)
              </span>
            </div>
